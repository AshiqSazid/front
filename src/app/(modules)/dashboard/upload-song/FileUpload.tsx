...
export default function FileUpload({setIsProcessing,isProcessing}: { setIsProcessing: (isProcessing: boolean) => void, isProcessing: boolean }) {
  ...
  const uploadFile = async (file: File) => {
    ...
    const progressInterval = setInterval(() => {
      if (currentProgress < 100) {
        ...
        setUploadStatus(prev => ({ ...prev, [file.name]: `Uploading... ${currentProgress}%` }));
      }
    }, 100);
    ...
    try {
      ...
      formData.append("file", file); // Upload entire file
      formData.append("originalname", file.name);
      formData.append("uniqueId", generateUniqueId(file.name));
  ...
  //   let chunkNumber = 1; // Start counting from 1
  //   let currentProgress = 0;
  ...
  //     // Loop until chunkNumber <= totalChunks (1-based)
  //     while (chunkNumber <= totalChunks) {
  //       const start = (chunkNumber - 1) * chunkSize; // Adjust for 1-based index
  //       const end = Math.min(start + chunkSize, file.size);
  //       const chunk = file.slice(start, end);
  
  //       const formData = new FormData();
  //       formData.append("file", chunk);
  //       formData.append("chunkNumber", chunkNumber.toString()); // Send 1-based number
    ...
    <TabsContent value="file" className="space-y-4 ">
      <div className="grid items-center gap-4 mt-4">
                  ...
                  <div key={index} className="flex items-center justify-between p-2 border rounded">
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between">
                        <p className="truncate">{file.name.length > 20 ? `${file.name.slice(0, 20)}...` : file.name}</p>
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {isUploading ? "Uploading..." :isProcessing ? "Analyzing..." : "Upload All"}
          </Button>
        </div>
      </div>
    </TabsContent>
